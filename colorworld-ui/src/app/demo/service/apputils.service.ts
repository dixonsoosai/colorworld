...
export const formatInvoiceData = (data: InvoiceSummary[]) => {
	return data.map(element => {
		return {
			...
			gntamt: element.gntxable + element.gncamt + element.gnsamt,
...
export const formatPurchaseBillData = (data: PurchaseBill[]) => {
	return data.map(element => {
		return {
			...
			artamt: parseFloat((element.arnamt + element.arcgst + element.arsgst).toFixed(2)),
			archqdte: element.archqdte ? getDMY(new Date(element.archqdte)) : "",
